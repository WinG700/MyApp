...
DWORD __stdcall acceptThread(void* pParam)
{
	...
	while (bConning)						                                //服务器的状态
	{
		...
		else//接受客户端的请求
		{
			...
			char sendBuf[16];
			/*char *pClientIP = */inet_ntop(AF_INET, (void*)&addrClient.sin_addr, sendBuf, 16);

			//char *pClientIP = inet_ntop(addrClient.sin_addr);
			u_short  clientPort = ntohs(addrClient.sin_port);
			cout << "Accept a client IP: " << sendBuf << "\tPort: " << clientPort << endl;
